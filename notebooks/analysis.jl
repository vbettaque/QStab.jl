...
# ╔═╡ 48df5b86-8585-11ee-09bf-abecaadc6abe
begin
	using CSV
	using DataFrames
	using Plots
...
begin
	symp_even = reshape(Matrix(data_stab[!, 1:16]), 1, :)
	symp_even_rref = reshape(Matrix(data_stab[!, 17:32]), 1, :)
	symp_odd = reshape(Matrix(data_stab[!, 33:48]), 1, :)
	symp_odd_rref = reshape(Matrix(data_stab[!, 49:64]), 1, :)
	ortho_even = reshape(Matrix(data_stab[!, 65:80]), 1, :)
	ortho_even_rref = reshape(Matrix(data_stab[!, 81:96]), 1, :)
	ortho_odd = reshape(Matrix(data_stab[!, 97:112]), 1, :)
	ortho_odd_rref = reshape(Matrix(data_stab[!, 113:128]), 1, :)
end
